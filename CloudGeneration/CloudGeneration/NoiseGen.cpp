	...
	for (int x = 0; x < N; x++)
	{
		for (int y = 0; y < N; y++)
		{
			for (int z = 0; z < N; z++)
			{
				...
				random = Remap(GetRandom(), 0, 1, 0.9, 1);
				R = (GetPerlinValue(pos, W_l) * random);
				...
				////Medium freq Worley
				//random = Remap(GetRandom(), 0, 1, 0.9, 1);
				//G = GetWorleyVaule(pos, W_m) * random;

				////High freq Worley
				//random = Remap(GetRandom(), 0, 1, 0.9, 1);
				//B = GetWorleyVaule(pos, W_h) * random;

				////Higest freq Worly
				//random = Remap(GetRandom(), 0, 1, 0.9, 1);
				//A = GetWorleyVaule(pos, W_hst) * random;
